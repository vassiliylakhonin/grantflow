...
def _architect_grounded_confidence_bonus(hit: Dict[str, Any], *, traceability_status: str) -> float:
    """Small bounded confidence bonus for strongly-grounded top-ranked evidence.

    Kept intentionally conservative to avoid swamping base semantic confidence.
    """
    if not isinstance(hit, dict):
        return 0.0

    rank = int(hit.get("rank") or 99)
    retrieval_rank = int(hit.get("retrieval_rank") or 99)
    ...
    if retrieval_rank == 1:
        bonus += 0.02
    elif retrieval_rank <= 3:
        bonus += 0.01

    bonus += min(0.015, max(0.0, retrieval_confidence) * 0.015)
    return round(min(0.06, bonus), 4)
...
def extract_architect_claim_records(
    ...
    *,
    max_claims: int = 24,
    max_high_priority_claims: int = 12,
) -> list[Dict[str, Any]]:
    claims = _extract_claim_strings(toc_payload, "toc")

    def _priority(path: str) -> int:
        p = path.lower()
        if p.endswith("project_goal") or ".project_goal" in p:
            return 5
        if "objective" in p or "outcome" in p or "result" in p:
            return 4
        if "description" in p:
            return 3
        return 2
    ...
    records.sort(key=lambda r: (int(r.get("priority") or 0), len(str(r.get("statement") or ""))), reverse=True)

    high_priority = [r for r in records if int(r.get("priority") or 0) >= 4][: max(0, max_high_priority_claims)]
    if len(high_priority) >= max_claims:
        return high_priority[:max_claims]

    selected = list(high_priority)
    for rec in records:
        if rec in selected:
            continue
        selected.append(rec)
...
def summarize_architect_claim_citations(
    *,
    ...
    claim_paths = [str(r.get("statement_path") or "") for r in claim_records if r.get("statement_path")]
    claim_paths_set = set(claim_paths)
    key_claims = [r for r in claim_records if int(r.get("priority") or 0) >= 4]
    key_paths_set = {str(r.get("statement_path") or "") for r in key_claims if r.get("statement_path")}

    arch_claim_citations = [
        c
        for c in citations
        if isinstance(c, dict)
        and c.get("stage") == "architect"
        and str(c.get("used_for") or "").startswith("toc_claim")
    ]

    covered_paths = {str(c.get("statement_path") or "") for c in arch_claim_citations if c.get("statement_path")}
    covered_paths &= claim_paths_set

    confident_paths = {
        str(c.get("statement_path") or "")
        for c in arch_claim_citations
        if c.get("statement_path") and str(c.get("citation_type") or "") == "rag_claim_support"
    }
    confident_paths &= claim_paths_set

    fallback_count = sum(1 for c in arch_claim_citations if str(c.get("citation_type") or "") == "fallback_namespace")

    traceability_complete = sum(1 for c in arch_claim_citations if str(c.get("traceability_status") or "") == "complete")
    traceability_partial = sum(1 for c in arch_claim_citations if str(c.get("traceability_status") or "") == "partial")
    traceability_missing = sum(1 for c in arch_claim_citations if str(c.get("traceability_status") or "") in {"", "missing"})
    traceability_gap = traceability_partial + traceability_missing

    threshold_considered = 0
    threshold_hits = 0
    for c in arch_claim_citations:
        if c.get("confidence_threshold") is None:
            continue
        threshold_considered += 1
        try:
            if float(c.get("citation_confidence") or 0.0) >= float(c.get("confidence_threshold") or 1.0):
                threshold_hits += 1
        except Exception:
            pass

    claims_total = len(claim_paths_set)
    key_total = len(key_paths_set)
    covered_total = len(covered_paths)

    return {
        "claims_total": claims_total,
        "key_claims_total": key_total,
        "claim_paths_covered": covered_total,
        "confident_claim_paths_covered": len(confident_paths),
        "fallback_claim_count": fallback_count,
        "key_claim_coverage_ratio": round((len(covered_paths & key_paths_set) / key_total), 4) if key_total else 0.0,
        "fallback_claim_ratio": round((fallback_count / claims_total), 4) if claims_total else 0.0,
        ...
        "traceability_gap_rate": round((traceability_gap / max(1, len(arch_claim_citations))), 4),
        "threshold_considered_count": threshold_considered,
        "threshold_hit_count": threshold_hits,
        "threshold_hit_rate": round((threshold_hits / threshold_considered), 4) if threshold_considered else 0.0,
    }
