...
def test_word_export_hides_diagnostics_when_disabled():
    ...
    assert "Export Contract Check" not in text
    assert "Quality Summary" not in text
    assert "Review Readiness" not in text
    assert "Citation Traceability" not in text
    assert "Critic Findings" not in text
    assert "Generated by: GrantFlow" not in text
    assert "USAID Results Framework" in text
