...
DEMO_PACK_ARCHITECT_RAG_ENABLED ?= 1
...
BENCHMARK_BASELINE_PILOT_DIR ?= $(PILOT_PACK_DIR)
BENCHMARK_BASELINE_METRICS_CSV ?=
...
pilot-quickcheck:
	./scripts/pilot_quickcheck.sh

pilot-quickcheck-auto:
	AUTO_JOB=1 ./scripts/pilot_quickcheck.sh

pilot-quickcheck-light:
	SKIP_QA_FAST=1 SKIP_DEMO_SMOKE=1 AUTO_JOB=1 ./scripts/pilot_quickcheck.sh

ci-pilot-quickcheck-smoke:
	SKIP_QA_FAST=1 SKIP_DEMO_SMOKE=1 AUTO_JOB=1 POLL_INTERVAL_SEC=1 ./scripts/pilot_quickcheck.sh

kz-ai-demo:
	$(PYTHON) scripts/kz_ai_training_demo.py
...
pilot-stack-up:
	docker compose $(PILOT_COMPOSE_ARGS) up -d --build

pilot-stack-down:
	docker compose $(PILOT_COMPOSE_ARGS) down

pilot-stack-logs:
	docker compose $(PILOT_COMPOSE_ARGS) logs --tail=150 api worker redis chroma
...
pilot-metrics:
	$(PYTHON) scripts/pilot_metrics.py \
		...
		$(if $(strip $(PILOT_METRICS_MEASURED_BASELINE_CSV)),--measured-baseline-csv $(PILOT_METRICS_MEASURED_BASELINE_CSV),) \
...
pilot-metrics-refresh: pilot-pack
	$(MAKE) pilot-metrics PILOT_METRICS_PILOT_DIR=$(PILOT_PACK_DIR) PILOT_METRICS_MEASURED_BASELINE_CSV=$(PILOT_METRICS_MEASURED_BASELINE_CSV) PILOT_METRICS_CSV_OUT=$(PILOT_METRICS_CSV_OUT) PILOT_METRICS_MD_OUT=$(PILOT_METRICS_MD_OUT)
...
benchmark-baseline:
	$(PYTHON) scripts/pilot_benchmark_baseline.py \
		--pilot-pack-dir $(BENCHMARK_BASELINE_PILOT_DIR) \
		$(if $(strip $(BENCHMARK_BASELINE_METRICS_CSV)),--metrics-csv $(BENCHMARK_BASELINE_METRICS_CSV),) \
		$(if $(strip $(BENCHMARK_BASELINE_CSV_OUT)),--csv-out $(BENCHMARK_BASELINE_CSV_OUT),) \
		$(if $(strip $(BENCHMARK_BASELINE_MD_OUT)),--md-out $(BENCHMARK_BASELINE_MD_OUT),)

benchmark-baseline-refresh: pilot-metrics-refresh
	$(MAKE) benchmark-baseline BENCHMARK_BASELINE_PILOT_DIR=$(PILOT_PACK_DIR) BENCHMARK_BASELINE_METRICS_CSV=$(BENCHMARK_BASELINE_METRICS_CSV) BENCHMARK_BASELINE_CSV_OUT=$(BENCHMARK_BASELINE_CSV_OUT) BENCHMARK_BASELINE_MD_OUT=$(BENCHMARK_BASELINE_MD_OUT)
